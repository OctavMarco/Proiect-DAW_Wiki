...
using ProPedia.Models;
using Microsoft.AspNet.Identity;

namespace ProPedia.Controllers
{
        ...
        public ActionResult Create([Bind(Include = "ID,Category,Title,Content")] Article article)
        {
            if (ModelState.IsValid)
            {
                ...
                foreach (var category in db.Categories)
                    if (category.Name.Equals(article.Category))
                        k = 1;

                if (k == 0)
                {
                    var cat = new Category();
                    cat.ID = db.Categories.Count() + 1;
                    cat.Name = article.Category;
                    db.Categories.Add(cat );
                }
                article.IdUser = User.Identity.GetUserId();
